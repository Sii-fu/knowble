...
/// FeedbackService handles all feedback-related operations with Supabase
/// Manages feedback submissions and retrieval for students
class FeedbackService {
  ...
  List<String> getFeedbackCategories() {
    return ['Course', 'Payment', 'Instructor', 'App UI', 'Other'];
  }
  ...
  Future<String?> updateFeedbackStatus({
    ...
    try {
      ...
      if (!isValidStatus(newStatus)) {
        return 'Invalid status value. Valid statuses are: ${getFeedbackStatuses().join(', ')}';
      }
  ...
  Future<List<Feedback>> getAllFeedback({
    ...
    try {
      ...
      final userRole = await getCurrentUserRole();
      if (userRole != 'admin') {
        print('❌ Only administrators can view all feedback');
        return [];
      }
